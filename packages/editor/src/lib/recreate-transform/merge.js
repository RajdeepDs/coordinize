...
export function mergeTransforms(
  ...
  if (rebase) {
    ...
    const conflicts = findConflicts(tr1Conflict, tr2Conflict),
      { inserted, deleted, conflictingSteps1, conflictingSteps2 } =
        createConflictingChanges(tr1Conflict, tr2Conflict);
...
function rebaseMergedTransform(
  doc,
  nonConflictingDoc,
  conflictingDoc,
  wordDiffs,
) {
  const trNonConflict = recreateTransform(
      doc,
      nonConflictingDoc,
      true,
      wordDiffs,
    ...
    trConflict = recreateTransform(
      nonConflictingDoc,
      conflictingDoc,
      false,
      wordDiffs,
    ),
    { inserted, deleted, conflictingSteps2 } = createConflictingChanges(
  ...
  return {
    tr: trNonConflict,
    merge: new Merge(trNonConflict.doc, changes, [], [], conflictingSteps2, {
      inserted,
      deleted,
    }),
  };
...
export class Merge {
    ...
    conflictingSteps1 = conflictingSteps1
      .map(([index, conflictStep]) => {
        ...
        if (mapped) {
          ...
          inserted = inserted.filter(
            (inserted) =>
              inserted.data.user !== 1 || inserted.data.index !== index,
          );
          deleted = deleted.filter(
            (deleted) =>
              deleted.data.user !== 1 || deleted.data.index !== index,
          );
          return false;
        }
      })
    ...
    conflictingSteps2 = conflictingSteps2
      .map(([index, conflictStep]) => {
        ...
        if (mapped) {
          ...
          inserted = inserted.filter(
            (inserted) =>
              inserted.data.user !== 2 || inserted.data.index !== index,
          );
          deleted = deleted.filter(
            (deleted) =>
              deleted.data.user !== 2 || deleted.data.index !== index,
          );
          return false;
        }
      })
  ...
  apply(user, index) {
    const step =
        user === 1
          ? this.conflictingSteps1.find(
              ([conflictIndex, conflictStep]) => conflictIndex === index,
            )[1]
          : this.conflictingSteps2.find(
              ([conflictIndex, conflictStep]) => conflictIndex === index,
            )[1],
      map = step.getMap(),
    ...
    if (user === 1) {
      conflictingSteps1 = conflictingSteps1
        .map(([conflictIndex, conflictStep]) =>
          conflictIndex === index
            ? false
            : [conflictIndex, conflictStep.map(map)],
        )
        .filter((step) => step);
      conflicts = conflicts.filter((conflict) => conflict[0] !== index);
    } else {
    ...
    const conflictingChanges = {
      inserted: this.conflictingChanges.inserted
        .filter(
          (inserted) =>
            inserted.data.user !== user || inserted.data.index !== index,
        )
        .map((inserted) => ({
          data: inserted.data,
          slice: inserted.slice,
          pos: map.map(inserted.pos),
        })),
      deleted: this.conflictingChanges.deleted
        .filter(
          (deleted) =>
            deleted.data.user !== user || deleted.data.index !== index,
        )
        .map((deleted) => ({
          data: deleted.data,
          from: map.map(deleted.from),
          to: map.map(deleted.to),
        })),
    };
  ...
  reject(user, index) {
    ...
    if (user === 1) {
      conflictingSteps1 = conflictingSteps1
        .map(([conflictIndex, conflictStep]) =>
          conflictIndex === index ? false : [conflictIndex, conflictStep],
        )
        .filter((step) => step);
      conflicts = conflicts.filter((conflict) => conflict[0] !== index);
    } else {
  ...
  applyAll(user) {
    ...
    while (steps.length) {
      ...
      if (mapped && !tr.maybeStep(mapped).failed) {
        changes = this.changes.addSteps(
          tr.doc,
          [tr.mapping.maps[tr.mapping.maps.length - 1]],
          { user },
        );
      }
    }
...
function automergeTransforms(tr1, tr2) {
  ...
  changes = changes.addSteps(tr.doc, tr.mapping.maps.slice(numberSteps1), {
    user: 2,
  });
...
function findContentChanges(tr) {
  ...
  const deleted = changes.deleted.map((deleted) => ({
    from: deleted.from,
    to: deleted.to,
    data: deleted.data,
  }));
...
function createConflictingChanges(tr1Conflict, tr2Conflict) {
  ...
  iter.forEach(({ steps, user }) =>
    steps.forEach(([index, step]) => {
      ...
      const invertedStepChanges = ChangeSet.create(stepResult.doc, {
        compare: (a, b) => false,
      }).addSteps(doc, [step.invert(doc).getMap()], { index, user });

      deleted = deleted.concat(
        invertedStepChanges.inserted.map((inserted) => ({
          from: inserted.from,
          to: inserted.to,
          data: inserted.data,
        })),
      ...
      inserted = inserted.concat(
        invertedStepChanges.deleted.map((deleted) => ({
          pos: deleted.pos,
          slice: deleted.slice,
          data: deleted.data,
        })),
