...
import { MarkdownParser, type ParseSpec } from "./MarkdownParser";
...
function getMarkdownSpec(extension: AnyExtension) {
  if (
    "markdownParseSpec" in extension.config === false ||
    typeof extension.config.markdownParseSpec !== "function"
  ) {
    return null;
  }
...
function getMarkdownToken(extension: AnyExtension) {
  if (
    "markdownToken" in extension.config === false ||
    typeof extension.config.markdownToken !== "string"
  ) {
...
export function createMarkdownParser(
  ...
  const tokens = extensions
    .filter(
      (extension) =>
        "markdownParseSpec" in extension.config &&
        typeof extension.config.markdownParseSpec === "function",
    )
