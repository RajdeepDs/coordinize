...
export const PasteHandler = Extension.create<PasteHandlerOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    const pasteParser = createMarkdownParser(
      schema,
      ...
      new Plugin({
        ...
        props: {
          ...
          handlePaste: (view, event: ClipboardEvent) => {
            ...
            const vscodeEditorData =
              event.clipboardData.getData("vscode-editor-data");
            const vscodeJSON = vscodeEditorData
              ? JSON.parse(vscodeEditorData)
              : undefined;
            ...
            if (vscodeLanguage && vscodeLanguage !== "markdown") {
              if (text.includes("\n") && !!state.schema.nodes.codeBlock) {
                ...
                const node = state.schema.nodes.codeBlock.create(
                  {
                    language: Object.keys(ALIAS_TO_LANGUAGE).includes(
                      vscodeJSON.mode,
                    )
                      ? vscodeJSON.mode
                      : null,
                ...
                if (tr.selection.from === tr.doc.content.size - 1) {
                  ...
                  tr.insert(tr.selection.from, para)
                    .setSelection(
                      TextSelection.near(
                        tr.doc.resolve(tr.selection.from + para.nodeSize + 1),
                      ),
                    )
                    .scrollIntoView();
                }
            ...
            if (
              ...
              if (singleNode?.type === this.editor.schema.nodes.paragraph) {
                ...
                singleNode
                  ? tr.replaceSelectionWith(singleNode, shiftKey)
