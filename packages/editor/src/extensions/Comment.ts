import { Mark, type Range, mergeAttributes } from "@tiptap/core";
    ...
    return [
                ...
                handleDOMEvents: {
                  mouseover(view, event) {
                    ...
                    const mark = node?.marks.find(
                      (mark) => mark.type === view.state.schema.marks.comment,
                    );
      ...
      new Plugin({
        ...
        view() {
          return {
            update(view) {
              const decos = commentKey
                .getState(view.state)
                .find(view.state.selection.to);
