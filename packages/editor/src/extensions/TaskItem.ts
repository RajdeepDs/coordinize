...
export interface TaskItemOptions
  extends Omit<TiptapTaskItemOptions, "onReadOnlyChecked"> {
  canEdit?(): boolean;
    ...
    return [
      new InputRule({
        ...
        handler: ({ state, range, match, chain }) => {
          if (
            isNodeActive(state, "bulletList") ||
            isNodeActive(state, "orderedList")
          ) {
            chain()
              .deleteRange(range)
          ...
          const wrapping =
            blockRange && findWrapping(blockRange, type, attributes);
