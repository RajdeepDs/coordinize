...
const inputRegex =
  /(?:^|\s)((?:1[0-2]|0?[1-9]):[0-5][0-9](?:am|pm)|(?:[01]?[0-9]|2[0-3]):[0-5][0-9])([.,!?\s])$/;
    ...
    return [
      new InputRule({
        ...
        handler: ({ state, range, match }) => {
          ...
          if (
            isMarkActiveInRange(state, "code", range) ||
            isNodeActiveInRange(state, "codeBlock", range)
          ) {
            return;
          }
          ...
          let parsedHours = Number.parseInt(hours, 10);
          ...
          if (match[1]) {
            ...
            tr.insert(insertionStart, this.type.create(attributes)).delete(
