...
import type { InlineAttachmentAttributes } from "./PostNoteAttachment";
...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    mediaGallery: {
      insertGallery: (
        gallery_id: string,
        attachments: InlineAttachmentAttributes[],
        pos?: number | "end",
      ) => ReturnType;
      appendGalleryItem: (
        gallery_id: string,
        attachment: InlineAttachmentAttributes,
      ) => ReturnType;
      updateGalleryItem: (
        optimistic_id: string,
        value: Partial<InlineAttachmentAttributes>,
      ) => ReturnType;
    ...
    return {
      insertGallery:
        ...
        ({ tr, state, dispatch }) => {
          ...
          const contentNodes = attachments.map((attachment) =>
            schema.nodes.mediaGalleryItem.create(attachment),
          );

          const nodes = [
            schema.nodes.mediaGallery.create({ id: galleryId }, contentNodes),
          ];
      ...
      appendGalleryItem:
        ...
        ({ tr, state, dispatch }) => {
          ...
          const galleryNode = findNodeAndPos(
            state,
            (n) => n.attrs.id === gallery_id && n.type.name === this.name,
          );
      ...
      updateGalleryItem:
        ...
        ({ tr, state, dispatch }) => {
          const match = findNodeAndPos(
            state,
            (n) =>
              n.attrs.optimistic_id === optimistic_id &&
              n.type.name === MediaGalleryItem.name,
          );
      ...
      removeGalleryItem:
        ...
        ({ tr, state, dispatch }) => {
          ...
          let newTr = tr.delete(
            galleryItemPos,
            galleryItemPos +
              mediaGalleryNode.child(galleryItemPos - galleryPos - 1).nodeSize,
          );
          ...
          if (updatedMediaGalleryNode.childCount === 0) {
            newTr = newTr.delete(
              galleryPos,
              galleryPos + updatedMediaGalleryNode.nodeSize,
            );
          }
      ...
      updateGalleryOrder: (gallery_id: string, newOrder: string[]) => {
        return ({ tr, state, dispatch }) => {
          ...
          tr.replaceWith(
            nodeStartPos,
            nodeStartPos + galleryNode.node.content.size,
            newContent,
          );
...
function getNewItems(
  newOrder: string[],
  initialItems: Map<string, ProseMirrorNode>,
) {
